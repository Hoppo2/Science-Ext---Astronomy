...
class _COMBO_Data(object):
    ...
    if exists("combo_data"):
        ...
        """
        CREATING A DATA MASK FOR PHOT_FLAG >= 8
        ...
        data = i_data[phot_mask]  # Application of the phot_flag mask to initial data array.
        ...
        percent_nan = {  # Calculation for the percentage of missing data in each of the used arrays.
        ...
        save_data_input = [{
            ...
            "Excl_Mcz_Slaon": Excl_Mcz_Sloan,
            "Excl_Mcz_Sloan": Excl_Mcz_Sloan,
            "Mcz_prop": Mcz_prop
        }, percent_nan]

        with open("data.json", 'w') as f:
            json.dump(save_data_input, f)
...
class Environment(_COMBO_Data):
    ...
    def density(self):
        ...
        :return:
        """
        a_mcz_mask = (self.a_min_z < self.Mcz_prop < self.a_max_z)

        a_mcz_mask = np.logical_and((self.a_min_z < self.Mcz_prop[:, 0]), (self.a_max_z > self.Mcz_prop[:, 0]))
        a_Mcz_prop = self.Mcz_prop[a_mcz_mask]
        a_galaxies = [a_Mcz_prop[1], a_Mcz_prop[1]]
        a_galaxies = [a_Mcz_prop[:, 1], a_Mcz_prop[:, 2]]
        ...
        observations = [len(a_galaxies[0]), len(a_galaxies[1])]
        df = [len(a_galaxies[0]) - 1, len(a_galaxies[1]) - 1]

        if self.b_min_z is not None and self.b_max_z is not None:
            b_mcz_mask = (self.b_min_z < self.Mcz_prop < self.b_max_z)
            b_Mcz_prop = self.Mcz_prop[b_mcz_mask]
            b_galaxies = [b_Mcz_prop[1], b_Mcz_prop[1]]
        ...
        a_density = len(a_galaxies[0]) / (a_x_range * a_y_range)
            ...
            return {"observations": observations, "df": df, "mean": a_mean, "var": a_var, "t-test_x": t_test_x,
                    "t-test_y": t_test_y}

        return {"observations": observations, "df": df, "mean": a_mean, "var": a_var}
...
class Cluster(_COMBO_Data):
    def __init__(self):
        ...
        """
        _Data.__init__(self)
        self.X_train = self.Incl_Mcz_Sloan
        _COMBO_Data.__init__(self)
...
if __name__ == "__main__":
    pass
    # print(Environment([[0.49930682, 0.45930682], [1.02141746, 0.98141746]]).density())
    # print(Environment([[1.02141746, 0.98141746]]).density())
