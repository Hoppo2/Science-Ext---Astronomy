...
class metrics(object):
    def __init__(self, class_instance, model):
    def __init__(self, model, class_instance=None):
        self.model = model
        self.cls_instance = class_instance

    def silhouette(self, generate_graph: bool = True):
    ...
    def inertia_graph(n_iterations: int):
        ...
        for n in k:  # TODO: Make sure this works
            _inertia_score.append(Cluster(test_size=0.2).kmeans(n=n, save=False).inertia_)
...
class ManualLabour(object):
    @staticmethod
    def best_model(n_iterations, n_clusters, test_size=0.2, save: bool = True):
    def best_model(n_iterations, n_clusters, save: bool = True):
        ...
        for x in range(0, n_iterations):
            if Cluster(test_size=_test_size).kmeans(n=n_clusters, save=False)[1] > best_model[1]:
                best_model = Cluster(test_size=_test_size).kmeans(n=n_clusters, save=False)
                logs[x] = ["best", best_model[1]]
        ...
        if save:
            pkl.save_model(model=best_model, name=f"kmeans_best_n-clusters:{n_iterations}_n-iterations:{n_clusters}")
...
if __name__ == "__main__":
    model = Cluster(test_size=0.2)
    cluster = model.kmeans(n=3, save=False)
    metrics(class_instance=model, model=cluster).silhouette(generate_graph=True)
