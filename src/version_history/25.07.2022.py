...
@dataclass
class _STAGES_Data(object):
    """"""
    if exists("stages_data"):
        with open(f'stages_data', 'rb') as handle:
        ...
        Excl_Mcz_Sloan: np.ndarray = save_data_output[0]["Excl_Mcz_Slaon"]
        ...
        data = i_data[phot_mask]  # Application of the phot_flag mask to initial combo_data array.
...
class Environment(_STAGES_Data):
class Environment(_COMBO_Data):
    ...
    def density_graph(self, bins: int = 0.002):
        ...
        x_values2 = self.Mcz_prop[:, 0]
        y_values2 = self.Mcz_prop[:, 3]
        print(y_values1)
        ...
        ax1.scatter(x_values2, y_values2, color=ax1_colour, s=2)
