...
class metrics(object):
    ...
    def silhouette(self, generate_graph: bool = True, data=None):
        ...
        if type(self.model) != KMeans:
            raise TypeError(
                f"Wasn't able to evaluate silhouette metric from the model type: {type(self.model)}. Requires KMeans model instead."
    ...
    def reachability(self):
        if type(self.model) != OPTICS:
            raise TypeError(
                f"Wasn't able to evaluate reachability metric from the model type: {type(self.model)}. Requires OPTICS model instead."
            )
        x_data = self.cls_instance.__getattribute__("X_train")
    ...
    def silhouette_graph(self, n_iterations: int):
        if type(self.model) != KMeans:
            raise TypeError(
                f"Wasn't able to evaluate inertia metric from the model type: {type(self.model)}. Requires KMeans model instead."
...
class _COMBO_Data(object):
    ...
    Excl_Mcz_Johnson: np.ndarray = np.array(
        list(zip(data["UjMAG"], data["BjMAG"], data["VjMAG"]
                 )))
    Incl_Mcz_Sloan: np.ndarray = np.array(
        list(zip(data["Mcz"], data["usMAG"], data["gsMAG"], data["rsMAG"]
                 )))
    Excl_Mcz_Sloan: np.ndarray = np.array(
        list(zip(data["usMAG"], data["gsMAG"], data["rsMAG"]
                 )))
    Incl_Mcz_Bessel: np.ndarray = np.array(
        list(zip(data["Mcz"], data["UbMAG"], data["BbMAG"], data["VbMAG"]
                 )))
    Excl_Mcz_Bessel: np.ndarray = np.array(
        list(zip(data["UbMAG"], data["BbMAG"], data["VbMAG"]
                 )))
    Incl_Mcz: np.ndarray = np.array(
        list(zip(data["Mcz"],
                 flux_to_luminosity(data["W462FE"], 426, data["Mcz"]),
                 flux_to_luminosity(data["W485FD"], 485, data["Mcz"]),
                 flux_to_luminosity(data["W518FE"], 518, data["Mcz"]),
                 flux_to_luminosity(data["W571FS"], 571, data["Mcz"]),
                 flux_to_luminosity(data["W604FE"], 604, data["Mcz"]),
                 flux_to_luminosity(data["W646FD"], 646, data["Mcz"]),
                 flux_to_luminosity(data["W696FE"], 696, data["Mcz"]),
                 flux_to_luminosity(data["W753FE"], 753, data["Mcz"]),
                 flux_to_luminosity(data["W815FS"], 815, data["Mcz"]),
                 flux_to_luminosity(data["W856FD"], 856, data["Mcz"]),
                 flux_to_luminosity(data["W914FD"], 914, data["Mcz"])
                 )))
    Excl_Mcz: np.ndarray = np.array(
        list(zip(flux_to_luminosity(data["W462FE"], 426, data["Mcz"]),
                 flux_to_luminosity(data["W485FD"], 485, data["Mcz"]),
                 flux_to_luminosity(data["W518FE"], 518, data["Mcz"]),
                 flux_to_luminosity(data["W571FS"], 571, data["Mcz"]),
                 flux_to_luminosity(data["W604FE"], 604, data["Mcz"]),
...
class Regression(_COMBO_Data):
    ...
    def polynomial(self, degree: int, save: bool = True, include_bias: bool = False):
        ...
        reg.fit(_x_train, y=self.y_train)  # TODO: Fix this to fit y peramater
