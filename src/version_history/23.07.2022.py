# TODO:
#  Analyse galaxy density in the clusters: No Statistical analysis on these
#  Analyse other possible environmental impacts: No statistical significance (t-test)
#   a
...
class _COMBO_Data(object):
    ...
    TODO: Exclude NaN values from the array
    """
    ...
    percent_nan = {
        ...
        "UbMAG": np.count_nonzero(np.isnan(data["UbMAG"]))/data["UbMAG"].size,
        "BbMAG": np.count_nonzero(np.isnan(data["BbMAG"]))/data["BbMAG"].size,
        "VbMAG": np.count_nonzero(np.isnan(data["VbMAG"]))/data["VbMAG"].size,
    ...
    Incl_Mcz_Johnson: np.ndarray = imp.fit_transform(np.array(
        list(zip(
    ...
    Excl_Mcz_Johnson: np.ndarray = imp.fit_transform(np.array(
        list(zip(
    ...
    Incl_Mcz_Sloan: np.ndarray = imp.fit_transform(np.array(
        list(zip(
    ...
    Excl_Mcz_Sloan: np.ndarray = imp.fit_transform(np.array(
        list(zip(
    ...
    Incl_Mcz_Bessel: np.ndarray = imp.fit_transform(np.array(
        list(zip(
            data["MC_z"], data["UbMAG"], data["BbMAG"], data["VbMAG"]
        ))
    ))

    Excl_Mcz_Bessel: np.ndarray = imp.fit_transform(np.array(
        list(zip(
    ...
    Mcz: np.ndarray = imp.fit_transform(np.array(
        list(zip(
            data["MC_z"]
        ))
    ))
    ...
    Mcz_prop: np.ndarray = imp.fit_transform(np.array(
        list(zip(
            data["MC_z"], data["x"], data["y"], data["ApD_Rmag"], data["MajAxis"], data["MinAxis"], data["dl"],
            data["mu_max"]
        ))
    ))
...
class Environment(_COMBO_Data):
    ...
    def density_graph(self, bins: int = 0.002):
        ...
        for i in array:
            ...
            raise TypeError("List must be in the form: [[]] or [[],[]]")
    ...
    def density(self):
        ...
        for i in self.Mcz_prop:
            if self.a_min_z < i[0] < self.a_max_z:
                a_galaxies[0].append(i[1])
                a_galaxies[1].append(i[2])
        a_mcz_mask = np.logical_and((self.a_min_z < self.Mcz_prop[:, 0]), (self.a_max_z > self.Mcz_prop[:, 0]))
        a_Mcz_prop = self.Mcz_prop[a_mcz_mask]
        a_galaxies = [a_Mcz_prop[:, 1], a_Mcz_prop[:, 2]]
        ...
        a_stdev = a_galaxies_array.std(axis=1)

        if self.b_min_z is not None and self.b_max_z is not None:
            b_galaxies = [[], []]
        a_var = a_galaxies_array.var(axis=1)

            for n in self.Mcz_prop:
                if self.b_min_z < n[0] < self.b_max_z:
                    b_galaxies[0].append(n[1])
                    b_galaxies[1].append(n[2])
        a_observations = [len(a_galaxies[0]), len(a_galaxies[1])]

            b_galaxies_array = np.array(b_galaxies)
            b_mean = b_galaxies_array.mean(axis=1)
            b_stdev = b_galaxies_array.std(axis=1)
        a_x_range = min(a_galaxies[0]) - max(a_galaxies[0])
        a_y_range = min(a_galaxies[1]) - max(a_galaxies[1])

            t_test_x = scipy.ttest_ind(a_galaxies[0], b_galaxies[0])
            t_test_y = scipy.ttest_ind(a_galaxies[1], b_galaxies[1])
        a_density = len(a_galaxies[0]) / (a_x_range * a_y_range)

            print(len(a_galaxies[0]), len(b_galaxies[0]))
            print()
            print(len(a_galaxies[1]), len(b_galaxies[1]))

            return {"mean": a_mean, "stdev": a_stdev, "t-test_x": t_test_x, "t-test_y": t_test_y}

        return {"mean": a_mean, "stdev": a_stdev}
    ...
    def average_maj_min_axis(self, equal_var=False):
        ...
        if self.b_min_z is not None and self.b_max_z is not None:
            ...
            minaxis_t_stat = scipy.ttest_ind(a_minaxis_values, b_minaxis_values, equal_var=equal_var)
            majaxis_t_stat = scipy.ttest_ind(a_majaxis_values, b_majaxis_values, equal_var=equal_var)
...
if __name__ == "__main__":
    print(Environment([0.73197994, 0.77197994], [0.13862278, 0.17862278]).density()["mean"],
          Environment([0.73197994, 0.77197994], [0.13862278, 0.17862278]).density()["t-test_y"])

    print(Environment([0.83964487, 0.87964487], [0.73197994, 0.77197994]).density()["mean"],
          Environment([0.83964487, 0.87964487], [0.73197994, 0.77197994]).density()["t-test_y"])

    print(Environment([0.13862278, 0.17862278], [0.83964487, 0.87964487]).density()["mean"],
          Environment([0.13862278, 0.17862278], [0.83964487, 0.87964487]).density()["t-test_y"])
